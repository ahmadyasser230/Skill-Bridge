...
// ✅ إعداد Socket.IO مع CORS خاص به
...
io.on('connection', (socket) => {
    ...
    socket.on('join-room', (roomId) => {
        socket.join(roomId);
        console.log(`👥 مستخدم انضم للغرفة: ${roomId}`);
    });
...
app.use((err, req, res, next) => {
    ...
    if (err && (err.type === 'entity.too.large' || err.statusCode === 413 || err.status === 413)) {
        return res.status(413).json({ message: 'حجم البيانات المرسلة أكبر من الحد المسموح' });
    }
